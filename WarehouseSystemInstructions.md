...
Now leave the `Create from scratch` option selected, enter `Surplus Agent_Firstname` as the name and enter the following into the Description field:
```
...
- Name: Secretary Agent_Firstname
- Description: 
```
...
- Define new agent
  - Display name: `TrafficAgentYourFirstname` (the name cannot contain a space). For example, `TrafficAgentSam`
